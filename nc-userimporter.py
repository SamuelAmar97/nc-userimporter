...
if not os.path.isfile(config_csvfile):
    print("ERROR!")
    print(
        "The csv-file (" + config_csvfile + ") you specified in you config.xml does not exist. Please save '" + config_csvfile + "' in main-directory of the script or edit your config.xml")
...
with codecs.open(os.path.join(appdir, config_csvfile), mode='r', encoding='utf-8') as csvfile:
    ...
    for row in readCSV:
        if (len(row) != 7):  # check if number of columns is consistent
            print("ERROR: row for user", html.escape(row[0]), "has", len(row),
                  "columns. Should be 7. Please correct your csv-file.")
            input("Press [ANY KEY] to confirm and end the process.")
            sys.exit(1)
        ...
        currentuser = [html.escape(row[0]), html.escape(row[1]), pass_anon, html.escape(row[3]), html.escape(row[4]),
...
if not config_GeneratePassword == 'yes':
    print(
        "ATTENTION: You have specified that users for whom no password has been entered will receive an e-mail to set a password for themselves. Please make absolutely sure that a correct e-mail address is entered for every user for whom no password has been set!")
...
with codecs.open(os.path.join(appdir, config_csvfile), mode='r', encoding='utf-8') as csvfile:
    ...
    for row in readCSV:
        ...
        print("Username:", html.escape(row[0]), "| Display name:", html.escape(row[1]), "| Password: ",
              ...
              html.escape(row[5]), "| Quota:", html.escape(row[6]), )
        ...
        if row[4]:
            grouplist = html.escape(row[4]).split(
                config_csvDelimiterGroups)  # Groups in the CSV-file are split by semicolon --> load into list
        else:
            grouplist = []
            ...
            if not response_xmlsoup.find('element'):
                try:
                    ...
                    groupresponse = requests.post(
                        config_protocol + '://' + config_adminname + ':' + config_adminpass + '@' +
                ...
                response_xmlsoup = BeautifulSoup(groupresponse.text, "html.parser")
                print('Create group "' + group.strip() + '": ' + response_xmlsoup.find(
                    'status').string + ' ' + response_xmlsoup.find('statuscode').string +
        ...
        if row[5]:
            groupadminlist = html.escape(row[5]).split(
                config_csvDelimiterGroups)  # Groupadmin Values in the CSV-file are split by semicolon --> load into list
            try:
        ...
        logfile.write("\nUSER: " + html.escape(row[0]) + "\nTIME: " + time.strftime("%d.%m.%Y %H:%M:%S",
                      ...
                      "\nRESPONSE: " + response_xmlsoup.find('status').string + ' ' + response_xmlsoup.find(
            'statuscode').string +
                      ' = ' + response_xmlsoup.find('message').string + "\n")
        logfile.close()
        ...
        if response_xmlsoup.find('statuscode').string == "100":
            # generate qr-code
            qr.add_data("nc://login/user:" + html.escape(row[0]) + "&password:" + html.escape(
                row[2]) + "&server:https://" + config_ncUrl)
